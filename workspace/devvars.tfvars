# =============================================================================
# Configuration Variables for AWS and Databricks
# =============================================================================

# -----------------------------------------------------------------------------
# AWS Variables
# -----------------------------------------------------------------------------

# AWS account ID where resources will be deployed
aws_account_id = ""

# AWS region for deployment
region = "us-east-2"

# -----------------------------------------------------------------------------
# Databricks Variables
# -----------------------------------------------------------------------------

# Admin user email for workspace and catalog management
admin_user = ""

# Databricks account ID
databricks_account_id = ""

# Unity Catalog Metastore ID
metastore_id = ""

# -----------------------------------------------------------------------------
# Resource Configuration
# -----------------------------------------------------------------------------

# Prefix used for naming and tagging resources (e.g., S3 buckets, IAM roles)
resource_prefix = "devbyam"

# -----------------------------------------------------------------------------
# Network Configuration
# -----------------------------------------------------------------------------

# Network type: "custom" or "isolated" (see README.md for more information)
network_configuration = "isolated"

# Required when using network_configuration = "isolated"
vpc_cidr_range       = "10.0.0.0/18"
private_subnets_cidr = ["10.0.0.0/22", "10.0.4.0/22"]
public_subnets_cidr  = ["10.0.12.0/22", "10.0.16.0/22"]
sg_egress_ports      = [443, 2443, 6666, 8443, 8444, 8445, 8446, 8447, 8448, 8449, 8450, 8451]

# CUSTOM NETWORK CONFIGURATION
# Required when using network_configuration = "custom"
# Uncomment and configure the following variables for custom networking:
# custom_vpc_id             = ""
# custom_private_subnet_ids = ["", ""]
# custom_sg_id              = ""
# custom_relay_vpce_id      = ""
# custom_workspace_vpce_id  = ""

# -----------------------------------------------------------------------------
# Telemetry Access Configuration
# -----------------------------------------------------------------------------

#maps to the telemetry bucket prefix for each environment
#specifices which environment's telemetry data will be accessible in the workspace's telemetry volume
telemetry_bucket_env_prefix = "devbyam" 

#telemetry external location name created in telemetry_data - leave as null if not created
telemetry_location_name = "telemetry-data"
telemetry_bucket_name = "telemetry-data-byam" #name of your telemetry bucket

# -----------------------------------------------------------------------------
# Feature Flags
# -----------------------------------------------------------------------------

# If audit log delivery is already configured
audit_log_delivery_exists = true

# If a regional metastore exists
metastore_exists = false

# Enable the security analysis tool
enable_security_analysis_tool = false

# -----------------------------------------------------------------------------
# Optional Configuration
# -----------------------------------------------------------------------------

# CMK Admin ARN (required for non-root account CMK usage)
# cmk_admin_arn = ""

# Compliance Security Profile Setting
# Standards: https://docs.databricks.com/aws/en/security/privacy/security-profile#compliance-security-profile-overview
# NOTE: If you add HIPAA, it is your responsibility before you process PHI data to have a BAA agreement with Databricks.
enable_compliance_security_profile = false

# NOTE: This variable is necessary if and only if you do not set default compliance settings at the account level.
# Avaiable compliance standards found here: https://pkg.go.dev/github.com/databricks/databricks-sdk-go/service/settings#ComplianceStandard
compliance_standards = null

# If deploying to govcloud, is deployment "civilian" or "dod" shard
databricks_gov_shard = null
